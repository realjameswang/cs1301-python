...
def word_count(my_string):
    count = 0
    # initialize variable called space which contains " "
    # so that we can count the number of spaces in a string
    space = " "
    try:
        for i in my_string:
            # if i is space
            if i == space:
                # word count + 1
                count += 1
        # note that total number of words is one more than the number of spaces
        num = count + 1
        return num
    except:
        return "Not a string"
