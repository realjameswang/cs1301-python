...
def to_metric(string):
    ...
    if "quart" in string:
        result = float(string[0:string.find(" ")]) * quarts
    elif "pint" in string:
        result = float(string[0:string.find(" ")]) * pints
    elif "cup" in string:
        result = float(string[0:string.find(" ")]) * cups
    elif "ounce" in string:
        result = float(string[0:string.find(" ")]) * ounces
    elif "tablespoon" in string:
        result = float(string[0:string.find(" ")]) * tablespoons
    elif "teaspoon" in string:
        result = float(string[0:string.find(" ")]) * teaspoons
    elif "gallons" in string:
        result = float(string[0:string.find(" ")]) * gallons
    
    # return equivalent number of milliliters as a float
    milli = round(result, 2)
    return milli
    # round the result
